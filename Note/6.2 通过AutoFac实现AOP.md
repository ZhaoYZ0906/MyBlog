...
日志与缓存整体形式上差不多，都是编写一个过滤器并挂载到对应的注册对象中。如果的挂载范围较大（比如当前代码直接挂载到整个项目），则适合进行一些比较通用的操作，比如缓存、日志等；如果挂载范围较小，比如某一个类，则可以进行当前类公共数据校验等功能；如果挂载范围极小（比如针对某个控制器下的某个方法）建议直接使用原生的lActionFilter过滤器进行扩展。